...
with Cyvest() as inv:
    console = Console()
    console.print("\n[bold cyan]Semantic Default Directions Example[/bold cyan]\n")
    ...
    console.print(f"✓ RESOLVES_TO: [green]{domain.relationships[0].direction.value}[/green] (domain → IP)")
    ...
    console.print(f"✓ DOWNLOADED: [red]{malware.relationships[0].direction.value}[/red] (file ← URL)")
    ...
    console.print(f"✓ COMMUNICATES_WITH: [yellow]{host1.relationships[0].direction.value}[/yellow] (host ↔ host)")
    ...
    # No direction specified! Automatically gets INBOUND  
    console.print(f"✓ FROM: [red]{email.relationships[0].direction.value}[/red] (email ← sender)")
    ...
    # No direction specified! Automatically gets OUTBOUND
    console.print(f"✓ TO: [green]{email.relationships[1].direction.value}[/green] (email → recipient)")
    ...
    # No direction specified! Automatically gets OUTBOUND
    console.print(f"✓ CREATED: [green]{process.relationships[0].direction.value}[/green] (process → file)")
    ...
    # No direction specified! Automatically gets INBOUND
    console.print(f"✓ CHILD: [red]{child_process.relationships[0].direction.value}[/red] (child ← parent)")
    ...
    # No direction specified! Automatically gets BIDIRECTIONAL
    console.print(f"✓ RELATED_TO: [yellow]{artifact1.relationships[0].direction.value}[/yellow] (artifact ↔ artifact)")
    ...
    # No direction specified! Automatically gets INBOUND
    console.print(f"✓ DROPPED: [red]{dropped.relationships[0].direction.value}[/red] (file ← dropper)")
    
    console.print("\n[bold]Summary:[/bold]")
    console.print("• [green]OUTBOUND (→)[/green]: RESOLVES_TO, TO, CREATED, CONTAINS, BELONGS_TO")
    console.print("• [red]INBOUND (←)[/red]: DOWNLOADED, DROPPED, FROM, SENDER, CHILD, DERIVED_FROM")
    console.print("• [yellow]BIDIRECTIONAL (↔)[/yellow]: COMMUNICATES_WITH, RELATED_TO, DUPLICATE_OF")
    console.print("\n[dim]All directions were automatically determined - no explicit direction parameters![/dim]")
    
    # Save investigation
    save_investigation_json(inv, "semantic_defaults_demo.json")
    save_investigation_markdown(inv, "semantic_defaults_demo.md")
    
    console.print("\n✓ Investigation saved to semantic_defaults_demo.json and .md")
